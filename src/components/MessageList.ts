...
const MessageList: React.FC<MessageListProps> = ({ 
  ...
  const dimensions = useMemo(() => ({
    ...
    maxVisibleMessages: Math.max(
      TERMINAL_CONSTANTS.MIN_VISIBLE_MESSAGES,
      Math.floor((maxHeight - 6) / TERMINAL_CONSTANTS.MESSAGE_HEIGHT_ESTIMATE)
    )
  ...
  const handleScroll = useCallback((direction: 'up' | 'down', amount: number = 1) => {
  ...
  const renderMessage = useCallback((message: Message) => {
    const boxWidth = MessageUtils.calculateMessageBoxWidth(
      message, 
      terminalWidth, 
  ...
  // Render user message
  const renderUserMessage = useCallback((message: Message, boxWidth: number, lines: string[]) => 
  ...
  // Render bot message
  const renderBotMessage = useCallback((message: Message, boxWidth: number) => 
    React.createElement(Box, { 
      flexDirection: "column", 
